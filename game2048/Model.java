...
class Model extends Observable {
    ...
    boolean tilt(Side side) {
        ...
        for (int row = size() - 2; row >= 0; row--) {
            for (int col = size() - 1; col >= 0; col--) {
                Tile t = vtile(col, row, side);
                if (t != null) {
                    Tile s = vtile(col, row + 1,  side);
                    if (s != null && t.value() == s.value()) {
                        setVtile(col, row + 1, side, t);
                        _score = _score + 2 * t.value();
                        move(col, side);
                        changed = true;
                    }
                }
    ...
    /**  Initial movement of tiles wrt SIDE. Return true if change found. */
    boolean move(Side side) {
        ...
        for (int row = size() - 1; row >= 0; row--) {
            for (int col = size() - 1; col >= 0; col--) {
                ...
                if (t != null) {
                    int trow = row + 1;
                    while (trow < size() && vtile(col, trow, side) == null) {
                        trow = trow + 1;
                    }
                    trow = trow - 1;
                    if (trow != row) {
                        changed =  true;
                        setVtile(col, trow, side, t);
                    }
    ...
    /** Moves within a COL with respect to SIDE. */
    void move(int col, Side side) {
        for (int row = size() - 1; row >= 0; row--) {
            Tile t = vtile(col, row, side);
            if (t != null) {
                int trow = row + 1;
        ...
        if (!hasmove) {
            for (int row = size() - 1; row >= 0; row--) {
                for (int col = size() - 1; col >= 0; col--) {
                    Tile t = tile(col, row);
                    if (row + 1 < size()) {
                        if (t.value() == tile(col, row + 1).value()) {
                            hasmove = true;
                            break;
                        }
                    }
                    ...
                    if (col + 1 < size()) {
                        if (t.value() == tile(col + 1, row).value()) {
                            hasmove = true;
                            break;
                        }
                    }
